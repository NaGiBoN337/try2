...
export const WelcomeScreen: FC<WelcomeScreenProps> = observer(function WelcomeScreen(_props) {
  ...
  const createTable = () => {
    db.transaction((tx: SQLTransaction) => {
      ...
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS Vaccination (id INTEGER PRIMARY KEY AUTOINCREMENT, id_pet INTEGER, date TEXT, comment TEXT, frequency TEXT, drug_name TEXT, FOREIGN KEY (id_pet) REFERENCES pets(id));",
        [],
        (tx: SQLTransaction, result: SQLResultSet) => {
          console.log('Таблица "Vaccination" создана успешно!')
        },
        (tx: SQLTransaction, error: SQLError) => {
          console.log('Ошибка при создании таблицы "Vaccination":', error)
        },
      )
    })
  }
  ...
  return (
    <ScrollView>
          ...
          <TouchableOpacity onPress={() => handleButtonProfile(pet.id)} key={pet.id}>
            <Petprofile
              post={{
                ...
                frofileImg: { uri: `data:image/jpeg;base64,${pet.imgbase}` },
              }}
            ></Petprofile>
          </TouchableOpacity>
        ))
      ...
      <Button style={styles.Textbtn} onPress={handleButtonCreate}>
        {" "}
        Создать
      </Button>
